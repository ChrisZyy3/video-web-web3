		...
		<view class="browser" :style="{ paddingTop: statusBarHeight + 'px' }">
			<view class="browser-bar">
				...
				<view class="browser-avatar">
					<text class="browser-avatar-text">{{ walletType.icon }}</text>
				</view>
			</view>
				...
				<view class="content">
					...
					<view class="card wallet-card">
						<view class="wallet-head">
							...
							<text class="wallet-status">已连接</text>
						</view>
...
const countdown = ref('19:13')
const feeMode = ref('resource')
const minerFee = ref('0.37')
const walletName = ref('TokenPocket')

const wallet = ref({
	usdt: '1.00',
	trx: '0.91',
	address: 'TR1rsFStNdW1QS77DL9gMimHLSRbS1M57z',
	addressShort: 'TEKNuZ...1FaZ'
})

const order = ref({
	expireAt: Date.now() + 19 * 60 * 1000 + 13 * 1000,
	icon: ''
})
...
const contractAddress = 'TR1rsFStNdW1QS77DL9gMimHLSRbS1M57z'
...
const getContract = () => {
	...
	const abi = getAbi()
	// Web3 v4 的 Contract 是 class，必须用 new 调用
...
const loadOrder = () => {
	...
	if (data) {
		order.value = { ...order.value, ...data, icon: id.value}
		if (data.expireAt) updateCountdown()
	}
	const selectedWallet = uni.getStorageSync('selectedWallet')
	if (selectedWallet?.name) walletName.value = selectedWallet.name
}
...
const handlePay = async () => {
	...
	if (!web3) {
		uni.showToast({ title: '请在钱包浏览器中打开', icon: 'none' })
		return
	}
	try {
		...
		await transferUSDT(contract, toAddress, amount)
	} catch (error) {
...
const transferUSDT = async (contract, toAddress, amount) => {
	const accounts = await web3.eth.getAccounts()
	const tx = await contract.methods.transfer(toAddress, amount).send({ from: accounts[0] })
	console.log('交易成功:', tx)
}
const walletType = uni.setStorageSync('wallet')
onLoad((options) => {
